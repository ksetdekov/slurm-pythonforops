...
def main():
    ...
    # часть задания про получение данных у меня не получилась, я не понимаю как в какой момент считывать данные из базы и для чего
    # реализацию составления отчета чтобы в нее вклиниться я поменять тоже не понял как, там я все делал через пандас
    exit(1)
